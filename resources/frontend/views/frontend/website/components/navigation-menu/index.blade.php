    ...
    <ul class="navbar-items flex items-center justify-center flex-1 p-1 space-x-1 list-none group">
        @foreach ($items as $key => $item)
            ...
            @if ($item->children->isEmpty())
                <li>
                    <x-website::link
                        class="navigation-menu-item font-semibold inline-flex items-center justify-center px-4 transition-colors hover:text-sky-400 focus:outline-none disabled:opacity-50 disabled:pointer-events-none group w-max gap-0 ease-out duration-300"
                        :href="$item->getUrl()"
                        >
